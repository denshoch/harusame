...
class HarusameTest extends TestCase
{
    ...
    {
        $harusame = new Harusame(["tcyDigit" => 0]);
        $source =    '12ああああ34ああ457あああ89';
        $expected = $source;
        $this->is_same($source, $expected, $harusame);
    ...
    public function testTransformTextWithOptions()
    {
        ...
        $source = '!!ああああ!!!ああ!?あああ??';
        $expected = '<span class="tcy">!!</span>ああああ!!!ああ<span class="tcy">!?</span>あああ<span class="tcy">??</span>';
     ...
     * @test
     */
    public function invalidTcyDigitTypeThrowsException(): void
    {
        $this->expectException(IllegalArgumentException::class);
        $this->expectExceptionMessage("tcyDigit should be int.");
        new Harusame(["tcyDigit" => "2"]);
    }

    /**
     * @test
     */
    public function negativeTcyDigitThrowsException(): void
    {
        $this->expectException(IllegalArgumentException::class);
        $this->expectExceptionMessage("tcyDigit should be 0 or greater.");
        new Harusame(["tcyDigit" => -1]);
    }

    /**
     * @test
     */
    public function invalidAutoTextOrientationTypeThrowsException(): void
    {
        $this->expectException(IllegalArgumentException::class);
        $this->expectExceptionMessage("autoTextOrientation should be boolean.");
        new Harusame(["autoTextOrientation" => "true"]);
    }
    ...
    public function malformedXmlThrowsException(): void
    {
        ...
        $this->harusame->transform("<abc>aaa</efg>");
    }
    ...
    {
        $this->expectException(InvalidXMLException::class);
        $this->expectExceptionMessage("Error processing XML: DOMDocumentFragment::appendXML(): Entity: line 1: parser error : StartTag: invalid element name");
        $this->harusame->transform("<123>invalid tag</123>");
    }
}